...
interface Props {
  ...
  /** tambahan from parent */
  schoolName: string; // nama sekolah admin
}
...
export function EditGTKModalAdminSekolah({
  open,
  onOpenChange,
  gtk,
  onUpdated,
  ...
  async function handleSubmit() {
    ...
    const gtkNik = nik.trim(); // prevent race condition

    setSaving(true);
    try {
      const res = await fetch(
        `/api/admin-sekolah/gtk/${encodeURIComponent(gtkNik)}`,
        {
          method: "PATCH",
