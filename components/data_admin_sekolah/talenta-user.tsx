...
export default function DataSuperAdminTalentaPage({ role, userName }: { role: UserRole; userName: string }) {
  ...
  const loadGTKSubmissions = async () => {
    try {
      ...
      const res = await fetch(`/api/admin-sekolah/talent-submissions/nik/${nik}`, { method: "GET" });
  ...
  const loadTagOptions = async () => {
    try {
      const res = await fetch(`/api/admin-sekolah/talent-submissions/tags/${nik}`, { cache: "no-store" });
      if (!res.ok) return;
  ...
  const loadMe = async () => {
    try {
      const res = await fetch(`/api/admin-sekolah/gtk/${nik}`, { method: "GET" });
